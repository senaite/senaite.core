...
class Order(VariableSchemaSupport, BrowserDefaultMixin, BaseFolder):
    ...
    archetype_name = 'Order'
    schema = schema
    allowed_content_types = ('OrderItem',)
    immediate_view = 'base_view'
    content_icon = 'order.png'
    use_folder_tabs = 0
    global_allow = 0
    ...
    actions = (
        ...
        { 'id': 'edit',
          'name': 'Edit',
          'action': 'string:${object_url}/order_edit',
          'permissions': (ModifyPortalContent,),
        },
        # override print actions
        {'id': 'print',
         'name': 'Print order',
         'action': 'string:${object_url}/order_print',
         'permissions': (View,),
        },
        {'id': 'log',
         'name': 'Log',
...
def modify_fti(fti):
    for a in fti['actions']:
        if a['id'] in ('syndication', 'references',
                       'metadata', 'localroles'):
            a['visible'] = 0
    return fti
