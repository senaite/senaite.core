## Controller Python Script "labanalysisspec_mod"
...
##parameters=
##title=Modify lab analysis spec
##
...
if came_from == 'edit':
    uid = context.REQUEST.form['LabAnalysisSpecUID']
    ...
    existing_specs = {}
    for spec_proxy in context.portal_catalog(portal_type='LabAnalysisSpec'):
    ...
    for uid in st_uids:
        if existing_specs.has_key(uid):
            ...
            as_id = context.generateUniqueId('LabAnalysisSpec')
            context.invokeFactory(id=as_id, type_name='LabAnalysisSpec')
            as = context[as_id]
        as.edit(
            SampleType=uid,
            ResultsRange=analysisspecs,
...
if came_from == 'edit':
    transaction_note('Lab Analysis Spec modified successfully')
    message=context.translate('message_order_edited', default='Lab Analysis Spec ${st} successfully modified', mapping={'st': st_title}, domain='bika')
else:
    display_titles = ''
    for title in st_titles:
        if display_titles:
            ...
            display_obj = 'Lab Analysis Specs'
        else:
            display_titles = title
            display_obj = 'Lab Analysis Spec'

    transaction_note('Lab Analysis Spec created successfully')
