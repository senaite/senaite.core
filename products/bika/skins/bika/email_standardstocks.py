## Script (Python) "email_standardstocks"
...
if request.has_key('Contact_email_address'):
    ...
    msg = 'portal_status_message=Standard stock list sent to %s' % (
        contact_address)
else:
    ...
    msg = 'portal_status_message=Standard stock list sent to %s at %s' % (
...
request.RESPONSE.redirect('%s/standardstocks?%s' % (context.absolute_url(), msg))
