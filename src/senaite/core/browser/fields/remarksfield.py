...
class RemarksHistoryRecord(dict):
    ...
    def modified_by(self):
        return self.get("modified_by", "")

    @property
    def modified_ulocalized(self):
        if not self.modified:
            return ""
        return ulocalized_time(self.modified,
                               long_format=True,
                               context=api.get_portal(),
                               request=api.get_request(),
                               domain="senaite.core")
...
class RemarksField(ObjectField):
    ...
    def add(self, instance, value):
        """Add a new remark record for the given content
        :param instance: the instance of this field
        :param value: the content of the new remark
        :returns: the new remark record
        """
        record = self.to_history_record(value)
        # the record path of set() prepends and fires the events
        self.set(instance, record)
        return record
    ...
    def get_input_widget_attributes(self, instance, request):
        """Returns the JSON-encoded `data-*` attributes for the React widget.
        Called from the (restricted) skins widget template.
        """
        from senaite.core.api import remarks as remarks_api
        return remarks_api.get_widget_attributes(instance, self, request)
