...
class DisposeSamplesView(BrowserView):
    """Action URL for the sample "dispose" transition
    """
    template = ViewPageTemplateFile("templates/dispose_samples.pt")

    def __init__(self, context, request):
        super(DisposeSamplesView, self).__init__(context, request)
    ...
    def __call__(self):
        ...
        form_dispose = form.get("button_dispose", False)
        form_cancel = form.get("button_cancel", False)

        # Handle dispose
        if form_submitted and form_dispose:
            logger.info("*** DISPOSE ***")
    ...
    def dispose(self, sample, comment):
        """Dispose the sample
        """
        wf = api.get_tool("portal_workflow")
        try:
            wf.doActionFor(sample, "dispose", comment=comment)
            return True
