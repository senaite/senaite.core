...
class PrintView(BrowserView):
    ...
    def __call__(self):
        """ Entry point of PrintView.
            ...
            context.portal_type is a Worksheets and there are
        ...
        elif self.context.portal_type == "Worksheets" and \
    ...
    def getWSTemplates(self):
        ...
        for template_id in self._TEMPLATES_LIST:
            ...
            names = list(map(lambda p: p.capitalize(), name_parts[1].split("_")))
            template_filename = " ".join(names)
            out.append({
                "id": template_id,
    ...
    def renderWSTemplate(self):
        ...
        try:
            ...
            err_msg = _(
                u"error_load_template_worksheet",
                default=u"Unable to load the template",
            )
            rep_template = self._TEMPLATE_ERR % (wsid, err_msg, embedt, tbex)
    ...
    def _analysis_data(self, analysis):
        ...
        andict['specs'] = specs
        andict['formatted_result'] = analysis.getFormattedResult(specs=specs, sciformat=self.scinot, decimalmark=decimalmark)
        ...
        andict['formatted_uncertainty'] = format_uncertainty(
            analysis, decimalmark=decimalmark, sciformat=self.scinot)
