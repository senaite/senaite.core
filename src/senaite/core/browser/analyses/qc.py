...
from senaite.core.browser.analyses.view import AnalysesView
...
class QCAnalysesView(AnalysesView):
    ...
    def __init__(self, context, request, **kwargs):
        ...
        self.icon = self.portal_url + \
            "/++resource++senaite.core.images/referencesample.png"
    ...
    def folderitem(self, obj, item, index):
        ...
        if obj.portal_type == 'ReferenceAnalysis':
            ...
            if obj.getReferenceType() == 'c':
                imgtype = "<img title='%s' " \
                          "src='%s/++resource++senaite.core.images/control.png" \
                          "'/>&nbsp;" % (
                              antype, self.context.absolute_url())
            if obj.getReferenceType() == 'b':
                imgtype = "<img title='%s' " \
                          "src='%s/++resource++senaite.core.images/blank.png" \
                          "'/>&nbsp;" % (
                              antype, self.context.absolute_url())
            ...
            imgtype = "<img title='%s' " \
                      "src='%s/++resource++senaite.core.images/duplicate.png" \
                      "'/>&nbsp;" % (
                          antype, self.context.absolute_url())
