...
class window.WorksheetManageResultsView
  ...
  load_analysis_method_constraint: (analysis_uid, method_uid) =>
    ...
    if method_constraints[6] and method_constraints[6] != ""
      $(i_selector).after "<img uid='#{analysis_uid}' class='alert-instruments-invalid' src='#{@get_portal_url()}/++resource++senaite.core.images/warning.png' title='#{method_constraints[6]}'>"
