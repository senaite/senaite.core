...
class ASTMImporter(object):
    ...
    def get_sample_by_id(self, sid, default=None):
        """Query a sample by ID or Client Sample ID

        NOTE: This method queries for any review_state!
              -> Validation needs to be implementd by the importer method

        :param sid: Sample ID or Client Sample ID
        :param default: Default value if no Sample was found
    ...
    def get_instrument(self, name, serial=None, version=None):
        """Query instrument by model and serial
    ...
    def log(self, message, level="info"):
        ...
        if self.instrument and self.importlog is None:
            self.importlog = api.create(self.instrument, "AutoImportLog")
            self.importlog.setInstrument(self.instrument)
            self.importlog.setInterface(", ".join(self.get_sender()))
            self.importlog.setImportFile("ASTM")
            self.importlog.setLogTime(timestamp.isoformat())
        if self.importlog:
            results = self.importlog.getResults()
            if results:
                results += "\n"
            messages = "{}{}".format(results, message)
            self.importlog.setResults(messages)
        return message
