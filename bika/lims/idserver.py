...
def external_get_id(context, prefix, batch_size=None):
...
class DefaultBikaIDServer(object):
    ...
    def generateUniqueId(self):
        ...
        if self.context.portal_type == "AnalysisRequest":
            ...
            # There are possibly codes that require the '-Rx'.
            return fn_normalize(
                "%s%s-R%s" % (s_prefix,
        ...
        if self.context.portal_type == "SamplePartition":
            ...
            # There are possible codes that require the '-Px'.
        ...
        if self.context.bika_setup.getExternalIDServer():
            ...
            for d in prefixes:
                ...
                if self.context.portal_type == "Sample":
                    ...
                    new_id = str(
                        external_get_id(self.context, "%s%s-" % (prefix, year)))
                    if padding:
                        new_id = new_id.zfill(int(padding))
                    return '%s%s-%s' % (prefix, year, new_id)
                elif d['portal_type'] == self.context.portal_type:
                    prefix = d['prefix']
                    padding = d['padding']
                    new_id = str(
                        external_get_id(self.context, "%s%s-" % (prefix, year)))
            ...
            npt = id_normalize(self.context.portal_type)
            new_id = str(external_get_id(self.context, npt + "-"))
            ...
            for d in prefixes:
                if self.context.portal_type == "Sample":
                    ...
                    prefix = fn_normalize(self.context.getSampleType().getPrefix())
                    padding = self.context.bika_setup.getSampleIDPadding()
            ...
            prefix = id_normalize(self.context.portal_type);
