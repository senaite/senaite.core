...
def generateUniqueId(context):
    ...
    if context.bika_setup.getExternalIDServer():
        ...
        def cache_exists(caches,cache_name):
            #Checking if there is cached id for this prefix
            for cache in caches:
                if cache and cache.getKey()==cache_name:
                    return True,cache
            return False,""
        ...
        def next_id(prefix):
            ...
            #Getting all bikacache objects
            plone = context.portal_url.getPortalObject()
            caches=plone.listFolderContents(contentFilter={"portal_type": 'BikaCache'})
            exists,cache=cache_exists(caches,prefix)
            #If this cache doesn't exist, find next ID and create cache object
            if not exists:
                ...
                new_id = _id + 1
                _id=plone.invokeFactory("BikaCache",id=tmpID(),Key=prefix,Value=str(new_id))
                item = plone[_id]
                item.markCreationFlag()
                return str(new_id)
            #If exists, increase and return
            else:
                new_id=str(int(cache.getValue())+1)
                cache.edit(Value=new_id)
                return new_id
