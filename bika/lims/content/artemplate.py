...
schema = BikaSchema.copy() + Schema((
    ...
    ComputedField(
        "SamplePointUID",
        expression="context.Schema()['SamplePoint'].get(context) and "
                   "context.Schema()['SamplePoint'].get(context).UID() or ''",
    ...
    ComputedField(
        "SampleTypeUID",
        expression="context.Schema()['SampleType'].get(context) and "
                   "context.Schema()['SampleType'].get(context).UID() or ''",
        widget=ComputedWidget(
            visible=False,
        ),
    ),
                ...
                'Preservation': {
                    'colModel': [
                        {'columnName': 'preservation_uid',
                         'hidden': True},
                        {'columnName': 'Preservation', 'width': '30',
    ...
    ReferenceField(
        ...
        widget=ReferenceWidget(
            ...
            description=_(
                "The Analysis Profile selection for this template"),
            visible={'edit': 'visible', 'view': 'visible',
                     'add': 'visible',
                     'secondary': 'invisible'},
            catalog_name='bika_setup_catalog',
    ...
    RecordsField(
        ...
        widget=ARTemplateAnalysesWidget(
            label=_("Analyses"),
            description=_(
                "Select analyses to include in this template"),
        )
    ),
...
class ARTemplate(BaseContent):
    ...
    def remove_service(self, service):
        ...
        services = [item for item in services if
                    item.get('service_uid', '') != uid]
