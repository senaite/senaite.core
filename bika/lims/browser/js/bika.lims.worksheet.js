...
function WorksheetAddAnalysesView() {
	...
	function batchselector_dropdown_init() {
		...
		var options = {
			...
			select: function (event, ui) {
				...
				$('#batchselector').attr('uid', ui['item']['UID'])
				$('#batchselector').val(ui['item']['Title'])
				$('[name$="_BatchUID"]').val(ui['item']['UID'])
				return false
			}
		}
		$("#batchselector").combogrid(options)
	...
	function category_selected() {
		...
		$('[name="list_getCategoryUID"]').live("change", function () {
			// first the special 'any' option, forcing us to not care.
			val = $('[name="list_getCategoryUID"]').val()
			if (val == 'any') {
			...
			// then fire a json request for the services
			var request_data = {
			...
			window.bika.lims.jsonapi_read(request_data, function (data) {
				if (data.objects && data.objects.length > 0) {
					current = $('[name="list_Title"]').val()
					// assumes that service title == analysis title
					$('[name="list_Title"]').empty()
					$('[name="list_Title"]').append("<option value='any'>" + _('Any') + "</option>")
					for (i = 0; i < data.objects.length; i++) {
						if (data.objects[i] == current) {
							selected = 'selected="selected" '
						}
						else {
							selected = ''
						}
						var uid = data.objects[i]['UID']
						var title = data.objects[i]['Title']
						$('[name="list_Title"]')
						  .append("<option " + selected + "value='" + uid + "'>" + title + "</option>")
					}
				}
			})

		})
		$('[name="list_getCategoryUID"]').trigger("change");
	}
	...
	function search_button_click() {
		...
		$('.ws-analyses-search-button').live('click', function (event) {
			...
			if ($('[name="list_getClientUID"]').val()
			  && $('[name="list_getClientUID"]').val() != 'any') {
				options['list_getClientUID'] = $('[name="list_getClientUID"]').val()
			}
			// assumes that service title == analysis title
			if ($('[name="list_Title"]').val()
			  && $('[name="list_Title"]').val() != 'any') {
				options['list_Title'] = $('[name="list_Title"]').val()
			}
			if ($('[name="list_getCategoryUID"]').val()
			  && $('[name="list_getCategoryUID"]').val() != 'any') {
				options['list_getCategoryUID'] = $('[name="list_getCategoryUID"]').val()
			}
			if ($('[name="list_BatchUID"]').val()) {
				options['list_BatchUID'] = $('[name="list_BatchUID"]').val()
			}


			$("table.bika-listing-table")
			  .load(window.location.href, options,
...
function WorksheetAddQCAnalysesView() {
	...
	that.load = function () {
		...
		$("#worksheet_add_references .bika-listing-table tbody.item-listing-tbody tr")
		  .live('click',
				function (e) {
...
function WorksheetAddDuplicateAnalysesView() {
	...
	that.load = function () {
		...
		$("#worksheet_add_duplicate_ars .bika-listing-table tbody.item-listing-tbody tr")
		  .live('click',
				function () {
...
function WorksheetManageResultsView() {
	...
	function loadWideInterimsEventHandlers() {
		...
		$("#wideinterims_apply").click(function (event) {
			...
			$('tr[keyword="' + analysis + '"] input[field="' + interim + '"]').each(function (i,
																							  obj) {
		...
		$(".manage_results_header .instrument").change(function () {
			...
			$.ajax({
					   ...
					   success: function (data, textStatus, jqXHR) {
						   ...
						   $('select.listing_select_entry[field="Instrument"] option[value="' + instruid + '"]').parent().find('option[value="' + instruid + '"]').prop("selected",
																																										false);
						   $('select.listing_select_entry[field="Instrument"] option[value="' + instruid + '"]').prop("selected",
																													  true);
					   },
		...
		$('table.bika-listing-table select.listing_select_entry[field="Method"]').change(function () {
			...
			if (muid != '') {
				...
				window.bika.lims.jsonapi_read(request_data, function (data) {
					...
					$(instrselector).closest('tr').find('td.interim input').prop('disabled',
																				 !m_manualentry);
					$(instrselector).closest('tr').find('td.Result input').prop('disabled',
																				!m_manualentry);
					if (!m_manualentry) {
						// This method doesn't allow the manual entry of Results
						var title = _("Manual entry of results for method ${methodname} is not allowed",
					...
					window.bika.lims.jsonapi_read(request_data, function (asdata) {
						service = (asdata.objects && asdata.objects.length > 0)
						  ? asdata.objects[0]
						  : null;
						...
						$.ajax({
							...
							if (invalid.length > 0) {
								...
								if (valid) {
									...
									var title = _("Invalid instruments are not shown: ${invalid_list}",
												  {invalid_list: invalid.join(", ")});
									...
									var title = _("No valid instruments found: ${invalid_list}",
												  {invalid_list: invalid.join(", ")});
							...
							$(instrselector).val("");
							if (!m_manualentry) {
								var title = _("Unable to load instruments: ${invalid_list}",
											  {invalid_list: invalid.join(", ")});
								$(instrselector).parent().append('<img class="alert-instruments-invalid" src="' + window.portal_url + '/++resource++bika.lims.images/exclamation.png" title="' + title + '")">');
