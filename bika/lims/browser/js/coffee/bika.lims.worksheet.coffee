...
window.WorksheetAddAnalysesView = ->
  that = this
  ...
  that.load = ->
    ...
    $('.ws-analyses-search-button').live 'click', (event) ->
      `var i`
      `var field_name`
      `var value`
      ...
      while i < filter_indexes.length
        ...
        value = $(element).val()
        if value == undefined or value == null or value == 'any'
          i++
          continue
      ...
      while i < base_fields.length
        ...
        i++
      $.post(window.location.href, params).done (data) ->
