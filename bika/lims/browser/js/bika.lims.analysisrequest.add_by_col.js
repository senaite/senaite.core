...
function AnalysisRequestAddByCol() {
	...
	function filterByClient() {
		...
		for (var arnum = 0; arnum < parseInt($("#ar_count").val(),
											 10); arnum++) {
	...
	function destroy(arr, val) {
		for (var i = 0; i < arr.length; i++) if (arr[i] === val) arr.splice(i,
																			1);
		return arr;
	}
...
//            st_base_query = $.toJSON(st_base_query);
	...
	function toggleCat(poc, category_uid, arnum, selectedservices, force_expand,
					   disable) {
		...
		if ($(tbody).hasClass("expanded")) {
			...
			$(tbody).load(url, options, function () {
				...
				$("input[class='cb']").bind("change",
											service_checkbox_change);
				if (selectedservices != []) {
					recalc_prices(arnum);
	...
	function calc_parts_handler(arnum, data) {
		...
		for (var p in parts) {
			...
			for (var s in parts[p].services) {
				if (!parts[p].services.hasOwnProperty(s)) {
					continue;
				}
	...
	function add_Yes(dlg, element, dep_services) {
		...
		for (json_key in keyed_deps) {
			if (!keyed_deps.hasOwnProperty(json_key)) {
				continue;
			}
