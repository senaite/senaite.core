...
class Report(BrowserView):
    ...
    template = ViewPageTemplateFile(
        "templates/qualitycontrol_resultspersample.pt")
    ...
    def ResultOutOfRange(self, analysis):
        ...
        for name, adapter in adapters:
            if not spec:
                return False
            return adapter(specification=spec)
        return False
    ...
    def __call__(self):
        ...
        header = _("Results per sample")
        subheader = _(
            "Analysis results for per sample and analysis service")
        ...
        analysis_count = 0
        # TODO: Use portal_catalog in future versions
        ...
        for service_title in keys:
            ...
            for a in analyses[service_title]:
                ...
                if in_range and in_range.get('acceptable', ''):
                    in_shoulder_range_count += 1
                    a['Result'] = "%s %s" % (a['Result'], warning_icon)
                # result out of range
                elif in_range and in_range.get('out_of_range', ''):
