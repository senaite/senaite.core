...
class AnalysesView(BikaListingView):
    ...
    def __init__(self, context, request, **kwargs):
        ...
        # Initializing some very used catalogs
    ...
    def get_analysis_spec(self, analysis):
        ...
        if hasattr(analysis, 'getObject'):
            # It is a brain
            if analysis.getResultsRangeand and not\
    ...
    def folderitem(self, obj, item, index):
        ...
        if can_edit_analysis:
            ...
            if not items['calculation'] or \
               (items['calculation'] and self.interim_fields[obj.UID]):
                items['allow_edit'].append('retested')
        ...
        # TODO: can_edit_analysis so far, to work faster. Check in future
        # if can_view_result:
        if can_edit_analysis:
            attachments = ""
        ...
        if can_edit_analysis and result:
            full_obj = full_obj if full_obj else obj.getObject()
            item['Result'] = result
            ...
            fu = format_uncertainty(
                full_obj, result, decimalmark=dmk, sciformat=int(scinot))
            fu = fu if fu else ''
