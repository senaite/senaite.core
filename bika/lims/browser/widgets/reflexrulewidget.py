...
When the results become available, some samples may have to be added to the
 next available worksheet for reflex testing. These situations are caused by the
 indetermination of the result or by a failed test.
</p>
<p>
...
<p>
- Each reflex rule have to be bound to an analysis method using the drop-down
 list. Inside the reflex rule the user will be able to add actions for each
 analysis service belonging to the selected method.
</p>
<p>
- For each analysis service the user can introduce a range of values or a
 ...
 has the same discrete value.
</p>
<p>
- Using the 'more' button, it is possible to add more actions for the same
 result inside an analysis service or add new analysis services and results.
</p>
<p>
- If there is an analysis service with a defined range and rules but the user
...
<p>
- After defining the rule, the user can set the check-box in order to define
 whether the new analysis has to be added in a different worksheet.
</p>
<p>
- If the user doesn't select that option the new analysis will be added to the
 current worksheet (or without worksheet if the analysis does not belong to
 anyone).
</p>
<p>
- If the check-box is set and the user doesn't select an analyst the system
 will look for an open worksheet and it will add the analysis to that worksheet,
 without caring about the analyst. If there are no open worksheets, the system
 will create a new worksheet with an analyst (chosen by the system).
</p>
<p>
- If the check-box is set and the user defines an analyst, the system will look
 for the first worksheet assigned to the analyst. If there is no open worksheet
 for that analyst, the system will create a new worksheet assigned to the
 analyst.
</p>
<p>
So far there are only two reflex actions: duplicate and replace.

- Repeat an analysis means to cancel it and then create a new analysis with
 the same analysis service used for the canceled one (always working with the
 same sample).
</p>
<p>
- Duplicate an analysis consist on creating a new analysis with the same
 analysis service for the same sample. It is used in order to reduce the error
 procedure probability because both results must be similar.
</p>
<p>
- If there are more than one 'repeat' actions for the same result, the system
 will do a 'duplicate' instead of another 'repeat'.
"""
